    ...
    while ((pDirent = readdir(pDir)) != NULL)
    {
        ...
        if((strcmpi(fileOrFolder,".") != 0) && (strcmpi(fileOrFolder,"..") != 0))
        {
            ...
            if (isDirectory(completePath))
            {
                ...
                strcat(completePath,"\\");
                filesToPatchCount += countAllFiles(completePath);
            }
            else
            {
               filesToPatchCount++;
...
FILE* openFile(char* fileName,const char* mode)
{
    ...
    if (fp == NULL)
    {
        ...
        printf("  \n- The file is not located in the right location");
        printf("  \n- The file was accessed by this program in another menu and it wasn't closed (if that's the case, restarting the program can solve this issue)");
        ...
        printf("\n\nThe program will try to proceed without that file, but it will most likely not work properly");
...
void systemCopy(char* srcFileName, char* src_folder,char* dest_folder)
{
    ...
    //printf("\ncommand: \'%s\'\n\n", command);
    remove(concatenate(dest_folder,srcFileName));
    system(command);
    free(command);
