...
static const int ice_smod[4][4] = {{333, 313, 505, 369},
                                   {379, 375, 319, 391},
                                   {361, 445, 451, 397},
                                   {397, 425, 395, 505}};

/* XOR values for the S-boxes */
static const int ice_sxor[4][4] = {{0x83, 0x85, 0x9b, 0xcd},
                                   {0xcc, 0xa7, 0xad, 0x41},
                                   {0x4b, 0x2e, 0xd4, 0x33},
                                   {0xea, 0xcb, 0x2e, 0x04}};
...
static const int ice_keyrot[16] = {0, 1, 2, 3, 2, 1, 3, 0,
                                   1, 3, 2, 0, 3, 1, 0, 2};
...
static unsigned int gf_mult(register unsigned int a, register unsigned int b,
...
ICE_KEY *ice_key_create(int n) {
  ...
  if ((ik->ik_keysched =
           (ICE_SUBKEY *)malloc(ik->ik_rounds * sizeof(ICE_SUBKEY))) == NULL) {
    free(ik);
    return (NULL);
  }
...
void ice_key_encrypt(const ICE_KEY *ik, const unsigned char *ptext,
                     unsigned char *ctext) {
  register int i;
...
void ice_key_decrypt(const ICE_KEY *ik, const unsigned char *ctext,
                     unsigned char *ptext) {
  register int i;
  register unsigned long l, r;
...
static void ice_key_sched_build(ICE_KEY *ik, unsigned short *kb, int n,
                                const int *keyrot) {
  int i;
...
void ice_key_set(ICE_KEY *ik, const unsigned char *key) {
  ...
  for (i = 0; i < ik->ik_size; i++) {
    ...
    ice_key_sched_build(ik, kb, ik->ik_rounds - 8 - i * 8, &ice_keyrot[8]);
  }
}
...
int ice_key_key_size(const ICE_KEY *ik) { return (ik->ik_size * 8); }

/*
 * Return the block size, in bytes.
 */

int ice_key_block_size(const ICE_KEY *ik) { return (8); }
